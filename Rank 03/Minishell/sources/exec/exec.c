...
/*   Updated: 2025/08/07 16:11:34 by tstevens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Attend tous les processus enfants et retourne 
le code de sortie du dernier processus correspondant à shell_data->pid.*/
static int	ft_collect_child_processes(t_data *shell_data)
{
	...
	if (WIFSIGNALED(final_status))
	{
		...
		if (WTERMSIG(final_status) == SIGQUIT)
...
/*Crée un processus enfant pour chaque commande de la pipeline,
exécute les commandes, puis collecte les statuts des processus enfants.*/
...
/*Vérifie l’état initial des commandes et prépare 
les pipes nécessaires à l’exécution de la pipeline.*/
...
/*Exécute une commande builtin dans le processus parent 
si elle est seule et sans redirection de pipe.*/
...
/*Gère l'exécution complète d’une pipeline de commandes :
prépare, exécute les builtins ou lance les processus enfants.*/
